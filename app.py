...
# ================= EMAIL CONFIG =================
ADMIN_ALERT_EMAIL = "hariofficial2865@gmail.com"   
SMTP_SERVER = "smtp.gmail.com"
SMTP_PORT = 587
SMTP_EMAIL = "diyamallaguntala@gmail.com"         
SMTP_PASSWORD = "azbs djrh izul thob"       
...
MANUAL_HISTORY_FILE = "uploads/manual_prediction_history.csv"
...
def update_logs_periodically():
    while True:
        ...
        for log in logs:
            if log['is_red_team']:
                reasons = []
                if log['usb_plug_count'] > 2:
                    reasons.append("High USB usage")
                if log['file_access_count'] > 60:
                    reasons.append("Excessive file access")
                if log['email_suspicious_count'] > 3:
                    reasons.append("Suspicious emails")
                if log['email_suspicious_ratio'] > 0.2:
                    reasons.append("High suspicious email ratio")

                message = f"""
LIVE ALERT: USER FLAGGED AS RED TEAM
...
- """ + "\n- ".join(reasons)
                ...
                send_alert_email(
                    subject="🚨 Insider Threat Alert (Live Monitoring)",
                    message=message
                )
...
def manual_prediction():
    ...
    if classifier is None or regressor is None or scaler is None:
        if not load_models():
            flash('ML models not loaded.', 'danger')
    ...
    if request.method == 'POST':
        try:
            ...
            if 'email_file' in request.files and request.files['email_file'].filename != '':
                email_content = request.files['email_file'].read().decode('utf-8')

            keyword_flags = {k: int(k in email_content.lower()) for k in SUSPICIOUS_KEYWORDS}
            ...
            features = np.array([
                ...
                email_total,
                email_suspicious,
                email_suspicious / email_total if email_total else 0,
                out_of_session,
            ...
            is_red_team = int(classifier.predict(features_scaled)[0])
            anomaly_score = float(regressor.predict(features_scaled)[0])
            probability = float(classifier.predict_proba(features_scaled)[0][1])
            ...
            if os.path.exists(MANUAL_HISTORY_FILE):
                df.to_csv(MANUAL_HISTORY_FILE, mode='a', header=False, index=False)
            else:
                df.to_csv(MANUAL_HISTORY_FILE, index=False)

            # ===== EMAIL ALERT =====
            if is_red_team:
                reasons = []
                if usb_plug > 2: reasons.append("High USB usage")
                if file_access > 80: reasons.append("Excessive file access")
                if email_suspicious > 3: reasons.append("Suspicious emails")
                if any(keyword_flags.values()): reasons.append("Sensitive email keywords detected")

                message = f"""
ALERT: USER FLAGGED AS RED TEAM
...
- """ + "\n- ".join(reasons)
                ...
                send_alert_email(
                    subject="🚨 Insider Threat Alert (Manual Prediction)",
                    message=message
                )

            results = {
                'is_red_team': is_red_team,
                'red_team_probability': probability,
                'anomaly_score': anomaly_score,
...
def prediction_history():
    ...
    history = []
    if os.path.exists(MANUAL_HISTORY_FILE):
        history = pd.read_csv(MANUAL_HISTORY_FILE).to_dict(orient='records')
